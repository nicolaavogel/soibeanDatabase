...
#setwd("C:/Users/navo/OneDrive - Danmarks Tekniske Universitet/Dokumenter/r_scripts/soibean/")

##### Adjust NCBI labels: 
# Function to extract scientific name from the input string
...
make_tree_plots <- function(tree_file, input_file){
  ...
  tr <- ggtree(tree) +                             
    ...
    scale_size_continuous(name = "Frequency", 
                          range = c(0.5, 3)) +  # Adjust the range as needed
...
#t1 <- make_tree_plots(tree_file, if1)
t2 <- make_tree_plots(tree_file, if2)
t3 <- make_tree_plots(tree_file, if3)
#t4 <- make_tree_plots(tree_file, if4)
t5 <- make_tree_plots(tree_file, if5)
t6 <- make_tree_plots(tree_file, if6)
t7 <- make_tree_plots(tree_file, if7)
...
# Function to create a banner plot
...
c2 <- ggarrange(banner2,p2, t2, ncol = 3, widths = c(0.5, 3, 7))
c3 <- ggarrange(banner3,p3, t3, ncol = 3, widths = c(0.5, 3, 7))
c5 <- ggarrange(banner5,p5, t5, ncol = 3, widths = c(0.5, 3, 7))
c6 <- ggarrange(banner6,p6, t6, ncol = 3, widths = c(0.5, 3, 7))
c7 <- ggarrange(banner7,p7, t7, ncol = 3, widths = c(0.5, 3, 7))

downALL <- ggarrange(c2,c3,c5,c6,c7, nrow = 6, common.legend = TRUE)
#ggsave("test.png", plot = test, width = 10, height = 20)

tree_file <- "Ursidae.new.dnd"
input_file <- "ancN4_2_10Result12.mcmc"
...
last <-ggarrange(banner1, tr, ncol = 2, widths = c(0.5, 10))


test <- ggarrange(last, downALL, ncol = 1, heights = c(5,10))
ggsave("downsample.png", plot = test, dpi = 300, width = 14, height = 21)
